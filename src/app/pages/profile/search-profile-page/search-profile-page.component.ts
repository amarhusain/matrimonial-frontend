...
@Component({
  ...
  templateUrl: './search-profile-page.component.html',
  styleUrl: './search-profile-page.component.scss'
})
export class SearchProfilePageComponent {
  ...
  onSubmitCriteriaSearch() {
    ...
    this.searchService.searchProfiles(this.criteria, 0, 10).subscribe({
      next: (response: PagedResponse<ProfileSearchDto[]>) => {
        ...
        this.router.navigate([`${APP_ROUTES.SEARCH_RESULT_PAGE}`]);
